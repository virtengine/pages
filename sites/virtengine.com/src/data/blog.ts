...
export const CURRENT_POSTS: BlogArticle[] = [
  {
    ...
    html: `
      ...
      <p>A tenant describes the resources it needs in an order. Providers compete with bids. A selected bid becomes a lease, usage records are submitted, and escrowed funds settle after the dispute window. These stages are explicit on-chain state rather than private records held by a marketplace operator.</p>
  ...
  },
  {
    ...
    html: `
      ...
      <p>The lifecycle begins with an order that expresses required resources and attributes. Providers observe open demand and submit bids. When a tenant selects a bid, the resulting lease fixes the relationship between the parties before a workload is provisioned.</p>
      ...
      <p>The marketplace settlement policy is designed with no marketplace commission deducted from provider proceeds. Network transaction fees and the costs of operating infrastructure still exist, but the protocol does not need a percentage toll on every lease to coordinate the market.</p>
